...
pub ProjectsPanel = {{ProjectsPanel}} {
        ...
        empty_state = <View> {
            ...
            <Label> {
                ...
                draw_text: { color: (THEME_COLOR_TEXT_DIM), text_style: <THEME_FONT_BOLD> { font_size: 10 } }
            }
            <Label> {
                text: "Run OpenCode in a project directory to see it here."
                draw_text: { color: (THEME_COLOR_TEXT_MUTED), text_style: <THEME_FONT_REGULAR> { font_size: 9 }, word: Wrap }
            }
        }
...
impl Widget for ProjectsPanel {
    ...
    fn draw_walk(&mut self, cx: &mut Cx2d, scope: &mut Scope, walk: Walk) -> DrawStep {
        ...
        self.view.view(&[id!(empty_state)]).set_visible(cx, is_empty);
