...
## 2026-02-27 – Eliminating Heap Churn and Borrow Checker Conflicts in Recursive Draw Loops
**Learning:** In Makepad components like `SessionsPanel`, performing $O(N)$ grouping (via `HashMap`), string formatting (`format!`), and `LiveId` generation inside a recursive `draw_tree` method causes massive heap churn and CPU overhead as it executes every frame. Furthermore, calling mutable methods like `begin_folder(cx, ...)` while iterating over a collection in `self` triggers borrow checker conflicts.
**Action:** Implement a one-time `rebuild_cache` method guarded by a `dirty` flag to pre-calculate all labels, project names, and `LiveId`s. In the `draw_walk`/`draw_tree` loop, use `std::mem::take(&mut self.cache_field)` to temporarily own the cached data, allowing iteration while calling other mutable methods on `self`. Restore the cache to the struct field immediately after the loop.
