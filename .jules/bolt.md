...
## 2026-06-12 – Lazy Caching of Expensive Computations during Streaming
**Learning:** Handlers that refresh UI caches (like `MessageProcessor::refresh_message_caches`) are often called on every character during text streaming. If these handlers also compute expensive, stable data like unified diffs or large-scale summaries, they cause significant O(N*M) overhead on every character append.
**Action:** Implement lazy caching for stable computed fields within update handlers. By checking if a cached value (e.g., `cached_full_diff`) is already present before re-computing, we avoid redundant expensive operations during high-frequency text updates.
