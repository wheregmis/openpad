    ...
    pub fn set_session_diffs(&self, cx: &mut Cx, diffs: &[openpad_protocol::FileDiff]) {
        if let Some(mut inner) = self.borrow_mut() {
                ...
                .find(|m| m.role == "assistant")
            {
                ...
                // Clear cached diff and refresh caches to pre-calculate the unified diff
                last_assistant.cached_full_diff.clear();
                MessageProcessor::refresh_message_caches(last_assistant);
